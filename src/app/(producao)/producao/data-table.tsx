...
// ==================== UTILITY FUNCTIONS ====================
const transformarParaEstruturaSAP = (
  ordemProducao: any,
  ConsumosSCADA: any,
  horario: Date,
  addedItemsWithLines: Map<string, number> // Add this parameter
) => {
  if (
    !ConsumosSCADA ||
    !Array.isArray(ConsumosSCADA) ||
    ConsumosSCADA.length === 0
  ) {
    throw new Error("Dados de consumo inválidos ou vazios");
  }

  const documentLinesByMateriaPrima = ConsumosSCADA.reduce(
    (acc: any, consumo: any) => {
      ...
      // First try to find in original ProductionOrderLines
      let baseLine = ordemProducao.ProductionOrderLines?.find(
        (line: any) => line.ItemNo === materiaPrima
      )?.LineNumber;

      // If not found in original lines, check if it was dynamically added
      if (baseLine === undefined && addedItemsWithLines.has(materiaPrima)) {
        baseLine = addedItemsWithLines.get(materiaPrima);
      }

      // If still undefined, assign the next available line number
      if (baseLine === undefined) {
        const maxLineNumber = Math.max(
          ...ordemProducao.ProductionOrderLines.map(
            (line: any) => line.LineNumber
          ),
          ...Array.from(addedItemsWithLines.values()),
          -1
        );
        baseLine = maxLineNumber + 1;
        console.warn(
          `BaseLine not found for ${materiaPrima}, assigning line number: ${baseLine}`
        );
      }
      ...
      if (acc[materiaPrima]) {
        ...
        acc[materiaPrima] = {
          ...
          BaseLine: baseLine, // Now this will always have a valid value
          BaseType: 202,
          Quantity: quantidade,
          BatchNumbers: [
            {
...
const generateZPLLabel = (ordemProducao: ProductionOrder, horario: Date) => {
  return `^XA${EGIQUIMICA_LOGO_ZPL}^FS^CF0,60^FO220,50^FDEgiquimica^FS^CF0,30^FO220,115^FDParque Industrial Guarda,Lt.10/15^FS^FO220,155^FDGuarda^FS^FO220,195^FDPortugal(PT)^FS^FO50,245^GB700,3,3^FS^CFD,50^FO50,265^FDOrdem Produc:^FS^CFA,45^FO50,315^FD${
    ordemProducao.DocumentNumber
...
const imprimirEtiqueta = async (
  ...
  try {
    ...
    if (printerStatus.isReadyToPrint) {
      ...
      throw new Error(
        `Erro na impressora: ${JSON.stringify(printerStatus.errors)}`
      );
    } else {
...
export default function DataTable<TData extends ProductionOrder, TValue>({
  ...
  const [addedItemsWithLines, setAddedItemsWithLines] = useState<
    Map<string, number>
  >(new Map());
  ...
  const entradaMercadoriaProducaoMutation = useEntradaMercadoriaProducao();
  ...
  const setError = useCallback(
    (operation: keyof typeof systemStatus, message: string) => {
      setSystemStatus((prev) => ({
        ...prev,
        [operation]: "error",
  ...
  const isConsumptionConfirmed = selectedOrder
    ? consumptionConfirmed.includes(selectedOrder.DocumentNumber)
    : false;

  const createInventoryEntryPayload = (
    ordemProducao: ProductionOrder,
    horario: Date
  ) => {
    // Generate batch number: ItemNo + DocumentNumber
    const batchNumber = `${ordemProducao.ItemNo}${
    ...
    return {
      ...
      DocumentLines: [
        {
          BaseType: 202, // Production Order base type
          BaseEntry: ordemProducao.AbsoluteEntry,
          Quantity: ordemProducao.PlannedQuantity,
          BatchNumbers: [
            {
  ...
  const handleStateChange = useCallback(
    (newState: string, ordemProducao: ProductionOrder) => {
      const estadoSap = ORDER_STATES.find(
        (estado) => estado.estado === newState
      )?.statusSAP;
      if (!estadoSap) return;
  ...
  const handleReactorChange = useCallback(
    (reactorName: string, ordemProducao: ProductionOrder) => {
  ...
  const handleConfirmValidation = useCallback(
    async (ordemProducao: ProductionOrder) => {
      try {
        setLoading("validation");
        clearMessages();

        // Check if we're closing the production order
        const isClosingOrder = estadoOP.estadoSAPAtual === "boposClosed";

        // If closing the order, create inventory entry first
        if (isClosingOrder && enviarMudancaSAP) {
          try {
            const inventoryPayload = createInventoryEntryPayload(
              ordemProducao,
              horario
            );

            console.log(
              "Creating inventory entry:",
              JSON.stringify(inventoryPayload, null, 2)
            );

            await entradaMercadoriaProducaoMutation.mutateAsync(
              inventoryPayload
            );

            toast.success(
              `Entrada de mercadoria criada para ${ordemProducao.ItemNo}`
            );
          } catch (inventoryError: any) {
            console.error("Inventory entry error:", inventoryError);
            throw new Error(
              `Erro ao criar entrada de mercadoria: ${
                inventoryError.message || "Erro desconhecido"
              }`
            );
          }
        }
        ...
        const isOrderCreated = criouOrdemSCADA.includes(
          ordemProducao.AbsoluteEntry
        );

        if (!isOrderCreated) {
          const TPI = TPIS.find(
            (tpi) => tpi.Receita === ordemProducao.ItemNo
          )?.TPI;

          await criarOrdemProducaoSCADAMutation.mutateAsync([
            {
              numeroOrdemProducao: ordemProducao.DocumentNumber,
              receita: ordemProducao.ItemNo,
              quantidade: ordemProducao.PlannedQuantity,
              reator: reatorOP.idReatorAtual,
              estado: ORDER_STATES.find(
                (estado) =>
                  estado.statusSAP === ordemProducao.ProductionOrderStatus
              )?.id,
          ...
          setCriouOrdemSCADA((prev) => [...prev, ordemProducao.DocumentNumber]);
          setSuccess("validation", "Ordem criada no SCADA com sucesso");
          toast.success(
            `Ordem ${ordemProducao.DocumentNumber} criada no SCADA com sucesso`
          );
        } else {
          await atualizarReatorOrdemSCADAMutation.mutateAsync({
            numeroOP: ordemProducao.DocumentNumber,
            Reator: reatorOP.idReatorAtual,
          });
          setSuccess("validation", "Reator atualizado com sucesso");
          toast.success(
            `Ordem ${ordemProducao.DocumentNumber}. Reator atualizado com sucesso`
          );
        }

        // Additional success message for closed orders
        if (isClosingOrder) {
          toast.success(
            `Ordem ${ordemProducao.DocumentNumber} fechada e produtos adicionados ao inventário`
          );
        }
        ...
        if (error.message?.includes("timeout")) {
          setError(
            "validation",
            "Timeout: Sistema não está respondendo. Tente novamente."
          );
        } else {
          setError(
            "validation",
    ...
    [
      ...
      reatorOP,
      horario,
      atualizarEstadoOrdemMutation,
      criarOrdemProducaoSCADAMutation,
      atualizarReatorOrdemSCADAMutation,
      entradaMercadoriaProducaoMutation, // ADD THIS DEPENDENCY
      table,
      setLoading,
      clearMessages,
      setSuccess,
  ...
  const handleConfirmConsumption = useCallback(
    async (ordemProducao: ProductionOrder) => {
      try {
        ...
        // Pass the addedItemsWithLines to the transform function
        const sapPayload = transformarParaEstruturaSAP(
          ordemProducao,
          fetchConsumosSCADA.data,
          horario,
          addedItemsWithLines // Add this parameter
        );

        console.log("SAP Payload:", JSON.stringify(sapPayload, null, 2)); // Debug log

        await new Promise((resolve) => setTimeout(resolve, 2000));
        await enviarConsumoSAPMutation.mutateAsync(JSON.stringify(sapPayload));

        setConsumptionConfirmed((prev) => [
          ...prev,
          ordemProducao.AbsoluteEntry,
        ]);
        ...
        if (error.message?.includes("timeout")) {
          setError(
            "consumption",
            "Timeout: Sistema SAP não está respondendo. Tente novamente."
          );
        } else {
          setError(
            "consumption",
            `Erro no consumo: ${error.message || "Erro desconhecido"}`
          );
        }
      }
    },
  ...
  const handlePrintLabel = useCallback(async () => {
    ...
    try {
      ...
      const result = await imprimirEtiqueta(
        generateZPLLabel(selectedOrder, horario)
      );
      setSuccess("printing", result.message);
    } catch (error: any) {
      setError(
        "printing",
        `Erro na impressão: ${error.message || "Erro desconhecido"}`
      );
    }
  ...
  useEffect(() => {
    ...
    resetReactorState();
    clearMessages();
    setAddedItemsWithLines(new Map());
  }, [rowSelection, resetOrderState, resetReactorState, clearMessages]);

  useEffect(() => {
    if (selectedOrder && numOP !== selectedOrder.DocumentNumber.toString()) {
      setNumOP(selectedOrder.DocumentNumber.toString());
    }
  }, [selectedOrder, numOP]);
            ...
            <OrderActionButtons
              ...
              consumptionDialog={
                <ConsumptionDialog
                  ...
                  addedItemsWithLines={addedItemsWithLines} // Add this prop
                  setAddedItemsWithLines={setAddedItemsWithLines} // Add this prop
                />
              }
            />
          </div>
