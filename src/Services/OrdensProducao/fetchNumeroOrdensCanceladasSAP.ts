...
export const useFetchNumeroOrdensCanceladasSAPProduction = (
  periodoData?: string,
  options?: {
    enabled?: boolean;
    staleTime?: number;
  ...
  return useQuery<number, Error>({
    queryKey: ["numeroOrdensCanceladasSAP", finalPeriodoData],
    ...
    queryFn: async ({ signal }) => {
      ...
      const { data } = await axios.get<number>(url, {
        params: {
          $filter: `CreationDate ge '${finalPeriodoData}' and ProductionOrderStatus eq 'boposCancelled' and U_Tipo eq 'R'`
        },
